...
pwd
# Should show: /Users/kushagrakumar/Desktop/citypulseAI/ai-ml
...
Create `/etc/systemd/system/citypulse-ai.service`:
...
Type=notify
User=your-username
WorkingDirectory=/path/to/citypulseAI/ai-ml
Environment="PATH=/path/to/citypulseAI/ai-ml/venv/bin"
ExecStart=/path/to/citypulseAI/ai-ml/venv/bin/gunicorn main:app \
  --workers 4 \
  --worker-class uvicorn.workers.UvicornWorker \
  --bind 0.0.0.0:8001
Restart=always
...
sudo systemctl enable citypulse-ai
sudo systemctl start citypulse-ai

# Check status
sudo systemctl status citypulse-ai

# View logs
sudo journalctl -u citypulse-ai -f
```

### **Step 17: Set Up Nginx Reverse Proxy**

Create `/etc/nginx/sites-available/citypulse-ai`:

```nginx
server {
    listen 80;
    server_name ai.citypulse.com;
...
```bash
# Enable site
sudo ln -s /etc/nginx/sites-available/citypulse-ai /etc/nginx/sites-enabled/
...
# Get certificate
sudo certbot --nginx -d ai.citypulse.com
