...
Create service file: `/etc/systemd/system/citypulse-processor.service`
...
Enable and start:
```bash
sudo systemctl enable citypulse-processor
sudo systemctl start citypulse-processor
sudo systemctl status citypulse-processor
```

View logs:
```bash
sudo journalctl -u citypulse-processor -f
```
...
Build and run:
```bash
docker build -t citypulse-processor .
