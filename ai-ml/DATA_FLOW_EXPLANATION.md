...
class FirebaseProducer:
    ...
    def send_event(self, event: Event) -> bool:
        ...
        doc_ref = self.db.collection("citypulse_events").document(event.id)
...
- Collection name: `citypulse_events`
...
class FirebaseClient:
    ...
    def get_recent_events(self, location, minutes=60):
        ...
        # Query Firestore
        query = self.db.collection("citypulse_events")
...
├── citypulse_events/          ← Written by data-ingestion
│   ├── evt_001
│   ├── evt_002
│   └── evt_003 ...
│
...
|----------------------|------------------|------------------|----------------------------------|
| `citypulse_events`   | data-ingestion   | ai-ml, backend   | Raw events from connectors       |
...
FIREBASE_PROJECT_ID=citypulse-ai
FIREBASE_PRIVATE_KEY_PATH=./firebase-credentials.json
FIREBASE_COLLECTION=citypulse_events  # Where to WRITE events
```
...
FIREBASE_EVENTS_COLLECTION=citypulse_events  # Where to READ events
...
# Output:
# ✓ Firebase initialized - Project: citypulse-ai
...
FIREBASE_COLLECTION=citypulse_events

# In ai-ml/.env:
FIREBASE_EVENTS_COLLECTION=citypulse_events
...
data-ingestion/pipelines (normalize)
    ↓
Firebase/citypulse_events (store)
    ↓
