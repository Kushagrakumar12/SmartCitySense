...
export function MoodMapView() {
  ...
  const { data: sentiments, isLoading } = useSentiments({ limit: 2000 });
  ...
  useEffect(() => {
    ...
    areaMap.forEach((data, area) => {
      ...
      // Count sentiments by category instead of averaging
      // This shows the dominant sentiment in each area
      const posCount = data.sentiments.filter(s => (s.sentiment_score || 0.5) > 0.6).length;
      const negCount = data.sentiments.filter(s => (s.sentiment_score || 0.5) < 0.4).length;
      const neutCount = data.sentiments.length - posCount - negCount;
      ...
      // Calculate average sentiment score for the area
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        <span className="text-green-600">+{area.positiveCount}</span> • 
                        <span className="text-yellow-600 ml-1">○{area.neutralCount}</span> • 
                        <span className="text-red-600 ml-1">−{area.negativeCount}</span>
                      </p>
                    </div>
                  </div>
