...
# Project root
PROJECT_ROOT="/Users/kushagrakumar/Desktop/SmartCitySense"
...
echo -e "${YELLOW}Starting data ingestion (mock mode, 100 events)...${NC}"
nohup bash -c "
    source venv/bin/activate
    python main.py --mode mock --events 100
...
# Start data processing in stream mode
echo -e "${YELLOW}Starting data processing (stream mode)...${NC}"
nohup bash -c "
    source venv/bin/activate
...
echo -e "  ${GREEN}✓${NC} Data Ingestion:  Completed (100 events sent to Firebase)"
